...
4. Read the concise result first. It contains the lookup time, provider cost, missing keywords, and an evidence file path. Read specific rows from that file only if the brief output is insufficient. Missing or null metrics are unknown, not zero.
