...
4. Read the concise result first. It contains the lookup time, provider cost, missing keywords, and an evidence file path. Read specific rows or monthly trends from that file only if the brief output is insufficient. Missing or null metrics are unknown, not zero. When `source` is `google_ads`, the market is not covered by DataForSEO Labs, so keyword difficulty and intent are always null there. Pass `--clickstream` only when the user wants refined volume; it doubles the cost.
...
6. Append one entry to `researchLog` in the context file: `{ "entryDate": "<today, YYYY-MM-DD>", "summary": "Keyword snapshot: <terms>, <market>. Verdict: <conclusion>. Evidence: <evidence path>" }`. Then run `agenticseo context` to validate it.
