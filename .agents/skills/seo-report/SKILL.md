...
1. Run `agenticseo reports`. It returns `reportsDirectory`, `templatesDirectory`, existing reports (newest first, with a summary preview) and report templates. If a report covers the same job, replace that file instead of adding a near-duplicate; a second report with the same title is refused. If a template fits the task, read its file and follow it.
...
5. Reply with the report path, a one-line verdict and the single top action. Do not paste the report into the chat.

To remove a report, delete its `.md` file and any `.html` export with the same name. Templates are Markdown files in `TEMPLATES_DIRECTORY` with the same shape: a `# Title` and a short description of what the report must contain.
