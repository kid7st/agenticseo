// Ported from OpenSEO src/server/lib/dataforseo/backlinks.ts at commit
// 0ffff93101043aad7600a3b6a499a0cd2887ef49.
// Copyright (c) 2026 Ben Senescu. MIT License; see LICENSES/OpenSEO.txt.
// Local changes: the invalid-summary console.error is replaced by the first
// validation issues in the thrown error.
import { z } from "zod";
import {
  normalizeBacklinksSpamFilterOptions,
  type BacklinksSpamFilterOptions,
} from "../backlinks/schemas.js";
import { createDataforseoBillingClassifier } from "../billingClassification.js";
...
export async function fetchBacklinksSummary(input: BacklinksRequest) {
  ...
  if (firstResult) {
    ...
    if (!parsed.success) {
      throw new AppError(
        "INTERNAL_ERROR",
        `DataForSEO backlinks-summary-live returned an invalid response shape: ${parsed.error.issues
          .slice(0, 3)
