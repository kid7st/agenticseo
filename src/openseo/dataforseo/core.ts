// Ported from OpenSEO src/server/lib/dataforseo/core.ts at commit
// 0ffff93101043aad7600a3b6a499a0cd2887ef49.
// Copyright (c) 2026 Ben Senescu. MIT License; see LICENSES/OpenSEO.txt.
// Local changes: a network failure and a non-JSON body are classified instead of
// escaping as raw errors; console.warn calls and dataforseoPostResponse (Lighthouse only)
// are removed; platform imports come from ../platform.js.
import { AppError, getRequiredEnvValue, type ErrorCode } from "../platform.js";
  ...
  return async (url: RequestInfo, init?: RequestInit): Promise<Response> => {
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      try {
        ...
        // A local CLI is often offline or behind a proxy; name the failure
        // instead of letting a bare "fetch failed" escape.
        const path = formatDataforseoRequestPath(url);
        throw new AppError(
          "UPSTREAM_UNAVAILABLE",
          `DataForSEO request failed on ${path}: ${error instanceof Error ? error.message : String(error)}`,
          { provider: "dataforseo", providerPath: path },
        );
      }
...
async function requestDataforseo<TTask extends DataforseoTaskLike>(
  ...
  try {
    ...
    throw new AppError(
      "INTERNAL_ERROR",
      `DataForSEO returned a response that is not JSON on ${path}`,
    );
  }
}
