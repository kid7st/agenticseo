// Ported from OpenSEO src/server/lib/dataforseo/envelope.ts at commit
// 0ffff93101043aad7600a3b6a499a0cd2887ef49.
// Copyright (c) 2026 Ben Senescu. MIT License; see LICENSES/OpenSEO.txt.
// Local changes: console.warn/console.error calls are removed; parseTaskItems puts
// the first validation issues in its error instead; platform imports come from
// ../platform.js.
import { z } from "zod";
import { AppError, type ErrorCode } from "../platform.js";
import type { DataforseoErrorClassifier } from "./core.js";
...
export function parseTaskItems<T extends z.ZodTypeAny>(
  ...
  if (!parsed.success) {
    throw new AppError(
      "INTERNAL_ERROR",
      `DataForSEO ${endpoint} returned an invalid response shape: ${parsed.error.issues
        .slice(0, 3)
