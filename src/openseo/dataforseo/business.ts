// Ported from OpenSEO src/server/lib/dataforseo/business.ts at commit
// 0ffff93101043aad7600a3b6a499a0cd2887ef49.
// Copyright (c) 2026 Ben Senescu. MIT License; see LICENSES/OpenSEO.txt.
// Local changes: business listing items are validated as records, and a malformed
// Q&A result or category row fails the call instead of being dropped silently.
import { z } from "zod";
import { dataforseoGet, dataforseoPost } from "./core.js";
import {
  assertOk,
  buildTaskBilling,
  isNoResultsTask,
...
// Business rows stay untyped records (callers pick fields by name), but the
// payload must at least be a list of objects before anything reads it.
const recordSchema = z.record(z.string(), z.unknown());

function invalidShape(endpoint: string, error: z.ZodError): AppError {
  return new AppError(
    "INTERNAL_ERROR",
    `DataForSEO ${endpoint} returned an invalid response shape: ${error.issues
      .slice(0, 3)
...
export async function fetchBusinessListingsSearch(input: {
  ...
  return {
    data: parseTaskItems("business_listings", task, recordSchema),
...
function combinedQuestionItems(results: unknown): Record<string, unknown>[] {
  ...
  return list.flatMap((result) => {
    ...
    if (!parsed.success) throw invalidShape("questions_and_answers", parsed.error);
    return [
      ...(parsed.data.items ?? []),
...
> {
  ...
  const rows = (task.result ?? []).flatMap((entry) => {
    ...
    if (!parsed.success) throw invalidShape("business_listings categories", parsed.error);
    return [
      {
