// Ported from OpenSEO src/server/features/ai-search/services/promptExplorer.ts at commit
...
// cache (a cache write failure fails the call); the cache key drops the
// organization and project; a model's error message is shown as is, since the
// output goes to the user's own terminal, not a browser; a rejected key stops
// every model like a billing issue.
import { OperationError } from "../../../errors.js";
import type { DataforseoClient } from "../../dataforseo/client.js";
import type { LlmResponseResult } from "../../dataforseoLlmSchemas.js";
import { AppError } from "../../platform.js";
import { buildCacheKey, type Cache } from "../../cache.js";
import { safeHostname, safeHttpUrl } from "../safeUrl.js";
import {
...
const AI_SEARCH_PROMPT_CACHE_NAMESPACE = "ai-search:prompt-response";

export async function explorePrompt(
  input: PromptExplorerInput,
  deps: { dataforseo: DataforseoClient; cache: Cache },
): Promise<PromptExplorerResult> {
  const { dataforseo, cache } = deps;
...
async function runModel(
  ...
  await args.cache.set(cacheKey, shaped, PROMPT_RESPONSE_TTL_SECONDS);
...
function mapErrorToResult(
  ...
  if (
    (reason instanceof AppError && reason.code === "AI_SEARCH_BILLING_ISSUE") ||
    (reason instanceof OperationError && reason.kind === "credentials")
  ) {
