// Ported from OpenSEO src/server/features/ai-search/services/brandLookup.ts at commit
// 0ffff93101043aad7600a3b6a499a0cd2887ef49.
// Copyright (c) 2026 Ben Senescu. MIT License; see LICENSES/OpenSEO.txt.
// Local changes: the caller passes the ledger-metered client and the project
// cache (a cache write failure fails the lookup); the cache key drops the
// organization and project, since the cache lives in the project; failed
// sub-calls are reported to the caller instead of logged; a rejected key stops
// the lookup like a billing issue, instead of degrading to empty platforms.
...
import {
  ...
  type LlmPlatform,
} from "../../dataforseo/shared.js";
import type { LlmCrossAggregatedItem } from "../../dataforseoLlmSchemas.js";
...
import {
  ...
  type CrossOutcome,
} from "./shareOfVoice.js";
import {
  shapeResult,
  type PlatformBundle,
  type PlatformOutcome,
} from "./brandLookupShaping.js";
import {
  brandLookupResultSchema,
  type BrandLookupInput,
  type BrandLookupResult,
} from "../../schemas/ai-search.js";
import { detectTarget } from "../../shared/targetDetection.js";
import {
  parseResearchTarget,
  type ResearchTarget,
...
/** What a lookup needs locally: the ledger-metered client, the project cache, and a sink for failed calls. */
type BrandLookupDeps = { dataforseo: DataforseoClient; cache: Cache; failures: string[] };
...
export async function getBrandLookup(
  ...
  const cached = brandLookupResultSchema.safeParse(await deps.cache.get(cacheKey));
  if (cached.success) {
    return {
      ...cached.data,
      query: input.query,
  ...
  const platformBundles: PlatformOutcome[] = settled.map((settledResult, i) => {
    ...
    deps.failures.push(`${platform}: ${messageOf(settledResult.reason)}`);
  ...
  if (allSucceeded && result.hasData) {
    await deps.cache.set(cacheKey, result, BRAND_LOOKUP_TTL_SECONDS);
  }
...
async function fetchPlatformData(
  ...
  { dataforseo, failures }: BrandLookupDeps,
  ...
  return {
    aggregated: fulfilledOr(aggregated, () => ({}), platform, "aggregated", failures),
    topPages: fulfilledOr(topPages, () => [], platform, "topPages", failures),
    mentions: fulfilledOr(mentions, () => [], platform, "mentions", failures),
    complete:
...
async function fetchCrossAggregated(
  ...
  return settled.map((result, i) => {
    ...
    failures.push(`${platform} share of voice: ${messageOf(result.reason)}`);
    ...
    if (
      ...
      ((result.reason instanceof AppError &&
        ...
        (result.reason instanceof OperationError &&
          result.reason.kind === "credentials"))
    ) {
      throw result.reason;
    }
  }
...
function fulfilledOr<T>(
  ...
  failures.push(`${platform} ${label}: ${messageOf(result.reason)}`);
  return fallback();
}
