// Ported from OpenSEO src/server/lib/audit/url-policy.ts at commit
// 0ffff93101043aad7600a3b6a499a0cd2887ef49.
// Copyright (c) 2026 Ben Senescu. MIT License; see LICENSES/OpenSEO.txt.
// Local changes: the start-URL DNS check uses node:dns instead of Cloudflare
// DNS-over-HTTPS and fails visibly when the host does not resolve; an allowPrivate
// option skips the blocked-host policy so a user can audit a local dev server.
...
async function hostnameResolvesToBlockedAddress(
  ...
  try {
    ...
    throw new AppError(
      "VALIDATION_ERROR",
      `Cannot resolve ${host} (${code}); check the URL`,
    );
  }
...
export type CrawlTargetPolicy = { allowPrivate?: boolean };
...
export function isCrawlableUrl(
  ...
  return policy.allowPrivate === true || !isBlockedHost(parsed.hostname);
}
...
export async function normalizeAndValidateStartUrl(
  ...
  if (!policy.allowPrivate) {
    if (isBlockedHost(parsed.hostname)) {
      throw new AppError("CRAWL_TARGET_BLOCKED", blockedMessage(parsed));
    }
...
function blockedMessage(url: URL): string {
  return `${url.hostname} is a local or private-network address; pass --allow-private to audit it`;
}
  ...
  for (let hop = 0; hop < START_URL_REDIRECT_HOPS; hop++) {
    ...
    try {
      response = await fetch(current, {
        method: "HEAD",
        redirect: "manual",
        headers: { "User-Agent": "AgenticSEO-Audit/1.0" },
    ...
    current = await normalizeAndValidateStartUrl(next.toString(), policy);
  }
  return current;
}
