...
- `query "SELECT ..."` runs one read-only SQL statement against the project database and returns up to 500 rows. Tables: `saved_keywords`, `saved_keyword_tags`, `saved_keyword_tag_assignments`, `keyword_metrics` (latest metrics per keyword and market), `audits`, `audit_pages`, `audit_issues` and `audit_lighthouse_results`; `SELECT sql FROM sqlite_schema` shows their columns.
