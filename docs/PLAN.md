...
- Exit: a shell-capable agent can create and reopen a project, update context, write and retrieve a report, and distinguish invalid input, missing credentials and provider failures without a Web or MCP server. A complete task works in Pi; other clients are verified in Phase 6.
