...
- Parity review (6c): the review found no upstream MCP tool, server function or skill without a local command, and closed four smaller gaps:
  ...
  A live pass on 2026-09-24 ran every command on a fresh project against DataForSEO, Search Console and GA4 for about $1.20. It read back every mutation (context, research log, saved keywords and tags, trackers, audits, Google connections) and checked every export file. It found two exit-code defects, both fixed: `ga4 use` crashed with exit 1 when the Admin API failed, and so did a Google token request that could not reach Google (now exit 4). The CLI now routes requests through `HTTPS_PROXY`/`HTTP_PROXY` from the environment (Node 24.14's `http.setGlobalProxyFromEnv`), so Google works behind a proxy without extra setup. All 42 rows are `verified`, Ahrefs Domain Rating last, once a real key confirmed its success path.
- Release (6d): v0.1.0 is a GitHub release built by `release.yml` from a version tag. A global install from a git URL cannot build, because npm installs the git dependency's build tools globally, so each release ships the packed CLI as `agenticseo.tgz`. In a clean `node:24` container on 2026-09-24, the README's install command gave a working `agenticseo` and the ten skills, linked into `~/.agents/skills`, and a paid `serp` call succeeded through the environment's proxy. `scripts/live-check.mjs` repeats the Phase 6c live pass before a release. **Phase 6 is complete.**
