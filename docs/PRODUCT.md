...
| --- | --- | --- | --- |
| Project site and market initialization | `src/serverFunctions/projects.ts`, `src/server/mcp/tools/create-project.ts` | in progress | `agenticseo init`; `test/project.test.ts` checks validation and refusal to overwrite. Settings are edited in `.agenticseo/project.json` and validated on every read; no list/archive, since a project is one directory. |
| Project positioning, competitors, key pages and preferences | `src/serverFunctions/projectContext.ts`, `src/server/features/project-context/services/ProjectContextService.ts` | in progress | `agenticseo context` validates `.agenticseo/context.json` with upstream's sections, caps and roles; `test/project.test.ts`. Agents edit the file instead of sending patch ops, so upsert-by-domain becomes a duplicate check. Research log and author stamps are not implemented. |
...
| SERP location and market resolution (name to `locationName`/location code) | `src/server/mcp/tools/search-serp-locations.ts`, `src/serverFunctions/serp-locations.ts`, `src/server/lib/dataforseo/serp-locations.ts` | not started | — `agenticseo init --location 2840` requires a hand-entered code. |
...
| Report templates and briefs | `src/server/mcp/tools/report-template-tools.ts`, `src/serverFunctions/reportTemplates.ts` | in progress | Templates are `.agenticseo/templates/*.md`, listed with a description by `agenticseo reports`; `test/reports.test.ts`. No brief generation or template id on reports. |
...
| Task skills: audits, competition, local SEO, links, reports and coaching | `plugins/openseo/skills/` (seo-audit, competitive-landscape, competitor-analysis, local-seo, link-prospecting, seo-report, seo-coach) | not started | — |
