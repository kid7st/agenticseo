#!/usr/bin/env node
// Lists upstream OpenSEO modules that docs/openseo-parity.md never names, so a capability
...
const product = readFileSync(new URL("../docs/openseo-parity.md", import.meta.url), "utf8");
const commit = product.match(/OpenSEO commit `([0-9a-f]{40})`/)?.[1];
if (!commit) throw new Error("docs/openseo-parity.md does not pin an upstream commit");
...
// Each group names the upstream units to check and how an openseo-parity.md mention is recognised.
const groups = [
  {
    name: "MCP tools",
...
for (const { name, units, mention = (unit) => unit.replace(/\.ts$/, "") } of groups) {
  ...
  console.log(`\n## ${name}: ${units.length - missing.length}/${units.length} named in openseo-parity.md`);
